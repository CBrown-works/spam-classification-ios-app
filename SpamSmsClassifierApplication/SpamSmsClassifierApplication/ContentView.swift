...
struct ContentView: View {
    @AppStorage("isDarkMode") private var isDarkMode = false  // used to stored property to determine if dark mode is on for visuals, saves this property to device so that the choice of dark mode or light mode is remembered/saved after user closes app
    
    @State private var messageText = "" // temporarily stores text a text from users or AI before being added to messages array
        ...
        if #available(iOS 16.0, *) {
            NavigationStack{ // provides foundation for navigation among pages within app
                List { // holds each page within a section that allows user to click labels to navigate
                    Section("MENU") {
                        ForEach(pages, id: \.name) { page in
                            NavigationLink(value: page) {// makes a link that allows lables to be able to navigate back and forth through app
                ...
                .navigationDestination(for:Page.self ) { page in
                    if page.name == "Spam SMS Classification"{// when a user clicks a label from section the if statements determine which page to display based on label name clicked
                        VStack{
                            HStack{
                            ...
                            ScrollView{
                                ForEach(messages, id: \.self){ message in // iterates through messages array to display ,essage in ui
                                    if message.contains("[SELF]"){ //used to distinguish text message from user an AI , if the next is from user it will be placed on right side of screen
                                        let newMessage = message.replacingOccurrences(of: "[SELF]", with: "") // removes label from user texr before displaying
                                        HStack{
                                            Spacer()
                                            Text(newMessage)
                                                .padding()
                            ...
                            }.rotationEffect(.degrees(180)) // the rotation effects flips messages to make messages display from bottom to top
